...
<body>
  ...
  <div id="header">
    ...
    <div id="body">
      ...
      <div class="h_d_top">
    	 <form action="pumphistory_2.jsp" method="get" name="dateform">
...
/* 			ResultSet rs0=stat.executeQuery("select count(*) from tb_sensorinfo_2 where DATE(date) between '"+stdate+"' and '"+enddate+"' and time between '"+stdatetime+"' and '"+enddatetime+"' order by id desc;" );	 */
/* 			String sql = "select count(*) from tb_sensorinfo_2 where DATE(date) between '"+stdate+"' and '"+stdate+"' order by id desc;" ; */
			String sql = "select count(*) from tb_sensorinfo_2 where DATE(date) between '"+stdate+"' and '"+enddate+"' ;";
			...
			ResultSet rs=stat.executeQuery("select * from tb_sensorinfo_2 where DATE(date) between '"+stdate+"' and '"+enddate+"' order by id limit "+StartRow+", "+PageSize);		// limit 50			
		
			/* Context ctx = new InitialContext();
			DataSource ds=(DataSource)ctx.lookup("java:comp/env/jdbc/pumpDB");
			Connection conn = ds.getConnection();
			Statement stat=conn.createStatement();
			ResultSet rs=stat.executeQuery("select * from tb_sensorinfo_2 order by id desc limit 50;"); */
...
<%
  ...
  if(PageNo != 1){
    ...
    %>
     <td>
     <% 
    out.print("<a href=pumphistory_2.jsp?PageNo=1&start="+stdate+"&end="+enddate+">首页 </a>");
    %>
    </td>
    <td>
    <%
    out.print("<a href=pumphistory_2.jsp?PageNo="+PrevStart+"&start="+stdate+"&end="+enddate+"><上一页</a>");
  }
   ...
   if(c <MaxPage){
     ...
     <td>
     <% 
        out.print("<a href=pumphistory_2.jsp?PageNo="+c+"&start="+stdate+"&end="+enddate+">"+c+"</a>");
     }else{
     %>
     </td>
     <td>
...
if(PageNo < MaxPage){ //如果当前页不是最后一页，则显示下一页链接
    ...
    <td>
    <%
    out.print("<a href=pumphistory_2.jsp?PageNo="+NextPage+"&start="+stdate+"&end="+enddate+">下一页></a>");
}
...
if(PageNo < MaxPage){
   ...
   <td>
	<% 
    out.print("<a href=pumphistory_2.jsp?PageNo="+MaxPage+"&start="+stdate+"&end="+enddate+">尾页</a>");
  }
  %>
  </td>
